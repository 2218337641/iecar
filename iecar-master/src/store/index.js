...
export default new Vuex.Store({
  state: {
    source:[
      ...
      // ["南方",65845],
      // ["北方",25984],
      // ["混合动力车",35215,21532,18654]
    ],
    series:[
      // [
      //   {
      //     name:'柴油车',
      //     data:[0.85,0.75,0.7,0.88,0.83,0.8],
      //     type:'line',
      //   },
      //   {
      //     name:'新能源车',
      //     data:[0.5,0.38,0.62,0.71,0.73,0.68],
      //     type:'line',
      //   }
      // ],
      // [
      //   {
      //     data:[3.8,3.5,4.1,2.8,2.6,3.0],
      //     type:'line'
      //   }
      // ]
    ],
  ...
  mutations: {
    charX1(state){
      var sourced = state.source
      var option = {
        title:{
          text:"二手车网站最新上架车辆地域分布饼图",
          x:'center'
        },
        tooltip: {
          trigger: 'item'
    ...
    //   var option = {
    //     title:{
    //       // 年限这种东西不好在网站找到，没法代表消费者?可在车管所找到，爬虫另算
    //       // 折价率的话原价是个问题
    //       // 折价率还要和车辆年限挂钩，这个比例怎么计算呢
    //       // 限定一个固定年限?2年?
    //       // 先做再说
    //       text:'中国南北地域油点二手车折价率折线图',
    //       subtext:'数据来源：瓜子二手车',
    //       x:'center'
    //     },
    //     legend:{
    ...
    // 年限有关的数据，暂定为用标准差表示，但与查到上架时间，分析无人问津与年限的关系可能会比较好
    // charX3(state){
    //   var series = state.series[1]
    //   option = {
    //     tooltip: {
    //       trigger: 'axis'
    //     },
    ...
    //           color: 'lightgreen'
    //         },
    //         data: [75, 50, 60, 55, 70, 65, 55,56,86,42]
    //       }
    ...
    getsncount(state,param){
      state.source=param;
    },
    getct(state,param){
      state.series=param;
    },
    getnorthird(state,param){
      state.northird=param
    },
    getsouthird(state,param){
      state.southird=param
    },
    getlinethird(state,param){
      state.linethird=param
    },
    getotherthird(state,param){
      state.otherthird=param
    },
    getlessen(state,param){
      state.lessen=param
    },
    getlineco(state,param){
      state.lineco=param
    },
    getlinepr(state,param){
      state.linepr=param
    },
    getlineti(state,param){
      state.lineti=param
    },
    getlinero(state,param){
      state.linero=param
    }
    // addCount(state){
    //   state.showCount++
